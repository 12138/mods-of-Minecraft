package Jimmynator.common.item.mage;
...
public class ItemStaff extends ItemJimmynator {
	...
	private boolean canDoMage(EntityPlayer player){
		if(player.capabilities.isCreativeMode)
			return true;
		else if(Jimmynator.proxy.isEquid(player, "mage")){
			if(this.attribute==1){
				if(player.inventory.hasItem(Jimmynator.fireRune.itemID)){
					player.inventory.consumeInventoryItem(Jimmynator.fireRune.itemID);
					return true;
				}
				else 
					return false;
			}else if(this.attribute==2){
				if(player.inventory.hasItem(Jimmynator.iceRune.itemID)){
					player.inventory.consumeInventoryItem(Jimmynator.iceRune.itemID);
					return true;
				}
				else 
					return false;
			}else if(this.attribute==3){
					if(player.inventory.hasItem(Jimmynator.lifeRune.itemID)){
						player.inventory.consumeInventoryItem(Jimmynator.lifeRune.itemID);
						return true;
					}
				else 
					return false;
			}else if(this.attribute==4){
					if(player.inventory.hasItem(Jimmynator.deathRune.itemID)){
						player.inventory.consumeInventoryItem(Jimmynator.deathRune.itemID);
						return true;
					}
				else 
					return false;
...
//		  if (FMLCommonHandler.instance().getEffectiveSide()==Side.CLIENT)
		  {
	        par2World.playSoundAtEntity(par3EntityPlayer, "mob.ghast.fireball", 1.0F, 1.0F / (new Random().nextFloat() * 0.4F + 1.2F));	        
